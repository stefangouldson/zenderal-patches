---
name: mod-new-plugin
description: Scaffold a brand-new Enderal SE patch plugin in this workspace — a Spriggit YAML folder with a correct EnderalSE header and a build/manifest.json entry (plus a FOMOD only if the install has options) — so it is buildable and CI-packaged from the first commit. Use when the user wants to start a new patch, add a second plugin, or add a compatibility patch for the Zenderal list.
---
...
8. **Does the install have anything to ask?** Optional or alternative plugins, optional assets, a
...
5. **Create a FOMOD only if the install has something to ask.** The default for a new release is
   **no FOMOD**: set `"fomod": false` on it in `build/manifest.json` and ship a plain archive — the
   ...
   - the release ships **optional or alternative plugins** the user picks between — a patch per
     third-party list mod, a "lite" variant, mutually exclusive presets;
