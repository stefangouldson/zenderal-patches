...
5. **Create the FOMOD stub** at `build/staging/<Release Name>/fomod/` — `info.xml` and
   ...
   sit at the **archive root**, which `build.ps1` handles.

   > **Or skip it.** Set `"fomod": false` on the release in `build/manifest.json` and ship a plain
   > archive — just the `.esp` at the root. That is the right shape when there is nothing to choose
   > and no requirement the user must be told at install time, which is usually the case for a
   > **replacement plugin** (`Apocalypse - Enderal Patch` does this). `build.ps1` then *refuses* to
   > build if a `fomod/` still exists for that release, so the two can't silently disagree.

   A single-plugin release with a FOMOD needs only:
